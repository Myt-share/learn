...
# from regular_term import multi_lcc
# from module.BTN import blindspotUNet as UNet
import torch
...
class Solver(object):
    def __init__(self, args):
        ...
        # self.model = EDM3(args).to(self.device)
        self.lr = args.lr
    ...
    def train(self):
        ...
        while not out:
            ...
            for i, (x, m) in enumerate(self.data_loader):
                ...
                # M1 = mask_sampling1(m, 0.05, '1').unsqueeze(1).float().to(self.device)
                ...
                for j in range(5):
                    ...
                    # loss2 = (x_rafine.matmul(fim) - fix.matmul(fim)).norm(1) / x.shape[0]
                    # x_recon_regular_loss = multi_lcc(x_recon, self.device) / x.shape[0]


                    Aloss = loss + loss1 + 0.15 * loss2 #+ 100 * x_recon_regular_loss
...
#
#         # Networks & Optimizers
#         self.model1 = UNet1(blindspot=True, method='Ours', dataset=args.dataset).to(self.device)
#         self.model2 = UNet2(blindspot=True, method='Ours', dataset=args.dataset).to(self.device)
#         self.model = Fuse(3, 64).to(self.device)
#         # self.model = EDM3(args).to(self.device)
#         self.lr = args.lr
#         self.beta1 = args.beta1
#         self.beta2 = args.beta2
...
#             for i, (x, m) in enumerate(self.data_loader):
#                 x = x.unsqueeze(1).float().to(self.device)
#                 m = m.unsqueeze(1).float().to(self.device)
#                 x_recon1 = self.model1(x)
#                 x_recon2 = self.model2(x)
#                 input = torch.cat((x, x_recon1, x_recon2), dim=1)
#
#                 out = self.model(input)
#                 loss = (out.matmul(m) - x).norm(1) / x.shape[0]
#                 # loss1 = (x_recon2.matmul(m) - x).norm(1) / x.shape[0]
#
#                 self.optim_model.zero_grad()
#                 loss.backward()
#                 self.optim_model.step()
...
#
#             if self.global_iter % self.print_iter == 0:
#                 print('[{}] loss:{:.7f}'.format(self.global_iter, _loss / (i + 1)))
#                 self.save_images(out, self.save_image + r'\recon1_epoch_%d.png' % self.global_iter)
#
#             if self.global_iter % self.ckpt_save_iter == 0:
#                 self.save_checkpoint(self.global_iter)
#
...
#
#             self.global_iter = checkpoint['iter']
#             self.model1.load_state_dict(checkpoint['model_states']['model'])
#             self.optim_model.load_state_dict(checkpoint['optim_states']['optim_model'])
#
#
#     def save_checkpoint(self, ckptname='last'):
...
#         im = imgs[0]
#         im = im.cpu().detach().numpy()
#         plt.imshow(im, cmap='gray', interpolation='nearest', aspect=0.2, vmin=-0.5, vmax=0.5)
#         plt.savefig(root)
#         # im = ((im/np.max(np.abs(im)) + 1)/2)*255
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--mask_dir', default=r'E:/muyuting/data/mask_AVO_30_1.npy', type=str, help='dataset directory')#E:/muyuting/data/
