...
class blindspotUNet(nn.Module):
    """Custom U-Net architecture for Self Supervised Seismic data interpolation.
    ...
    When in blindspot mode the following behaviour changes occur:
        * Input batches are duplicated for rotations: 90, 270. This increases the
          batch size by 2x. After the encode-decode stage the rotations are undone and
          concatenated on the channel axis with the associated original image. This 2x
    ...
    Args:
        in_channels (int, optional): Number of input channels, this will typically be either
            1 (Mono) or 3 (RGB) but can be more. Defaults to 1.
...
if __name__ == '__main__':
    ...
    model = blindspotUNet(blindspot=True, method='DSP', dataset='MAVGL12')
    y = model(a)
    print(y.shape)
